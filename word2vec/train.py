from pathlib import Path
import numpy as np

from preprocess import preprocess, map_words
from functions import generate_negative_pair, generate_negative_prob, generate_positive_context
from model import SkipGramNS


def build_training_pairs(tokens, window_size):
    position_pairs = generate_positive_context(tokens, window_size)
    center_ids = np.asarray([tokens[center_pos] for center_pos, _ in position_pairs], dtype=np.int64)
    pos_context_ids = np.asarray([tokens[ctx_pos] for _, ctx_pos in position_pairs], dtype=np.int64)
    return center_ids, pos_context_ids


def train_skipgram_ns(center_ids, pos_context_ids, neg_probs, vocab_size, embed_dim=20, scale=0.1, learning_rate=0.03, epochs=5, K=5, verbose=False):
    model = SkipGramNS(V=vocab_size, d=embed_dim, scale=scale)
    num_samples = len(center_ids)
    final_loss = None

    for epoch in range(1, epochs + 1):
        neg_samples = generate_negative_pair(neg_probs, K=K, num_samples=num_samples)
        order = np.random.permutation(num_samples)

        for idx in order:
            model.backward_pass(
                center_id=center_ids[idx],
                pos_context_id=pos_context_ids[idx],
                neg_context_ids=neg_samples[idx],
                learning_rate=learning_rate,
            )

        final_loss = model.forward_pass(center_ids, pos_context_ids, neg_samples)
        if verbose:
            print(f"epoch {epoch}/{epochs} - loss: {final_loss:.4f}")

    return final_loss


def grid_search(tokens, frequency, vocab_size, window_sizes, embed_dims, learning_rates, ks, scales, epochs_list, neg_powers, random_seed):
    best_config = None
    best_loss = float("inf")
    run_id = 0

    for window_size in window_sizes:
        center_ids, pos_context_ids = build_training_pairs(tokens, window_size)
        num_samples = len(center_ids)

        if num_samples == 0:
            continue

        for embed_dim in embed_dims:
            for learning_rate in learning_rates:
                for k in ks:
                    for scale in scales:
                        for epochs in epochs_list:
                            for neg_power in neg_powers:
                                run_id += 1
                                np.random.seed(random_seed)
                                neg_probs = generate_negative_prob(frequency, neg_power)
                                print(f"run {run_id}: ws={window_size}, pairs={num_samples}, d={embed_dim}, lr={learning_rate}, K={k}, scale={scale}, epochs={epochs}, power={neg_power}")

                                final_loss = train_skipgram_ns(
                                    center_ids=center_ids,
                                    pos_context_ids=pos_context_ids,
                                    neg_probs=neg_probs,
                                    vocab_size=vocab_size,
                                    embed_dim=embed_dim,
                                    scale=scale,
                                    learning_rate=learning_rate,
                                    epochs=epochs,
                                    K=k,
                                    verbose=False,
                                )

                                print(f"final_loss: {final_loss:.4f}")

                                if final_loss < best_loss:
                                    best_loss = final_loss
                                    best_config = {
                                        "window_size": window_size,
                                        "embed_dim": embed_dim,
                                        "learning_rate": learning_rate,
                                        "K": k,
                                        "scale": scale,
                                        "epochs": epochs,
                                        "neg_power": neg_power,
                                    }

    return best_config, best_loss

def main():
    np.random.seed(42)

    path = Path("data/wikitext2_tune.txt")
    text = path.read_text(encoding="utf-8")
    data = preprocess(text)
    word_to_id, id_to_word, tokens, frequency = map_words(data)

    print(f"vocab_size: {len(word_to_id)}")
    print(f"tokens: {len(tokens)}")

    best_config, best_loss = grid_search(
        tokens=tokens,
        frequency=frequency,
        vocab_size=len(word_to_id),
        window_sizes=[2, 5],
        embed_dims=[10, 20, 40],
        learning_rates=[0.01, 0.03, 0.05],
        ks=[3, 5],
        scales=[0.05],
        epochs_list=[20, 40],
        neg_powers=[0.75],
        random_seed=42,
    )

    print("best_config:", best_config)
    print(f"best_loss: {best_loss:.4f}")


if __name__ == "__main__":
    main()
